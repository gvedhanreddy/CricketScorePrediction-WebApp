...
namespace CricketAnalysis.Controllers
{
    ...
    [Route("[controller]")]
    /// <summary>
    /// Controller class of player has CRUD and Analysis API
    /// </summary>
    public class PlayerController : ControllerBase
    {
       ...
       /// <summary>
        ...
        {
            (string playerName, int averageRunsScored) = _playerRepository.GetAverageRunsScored(id);
            ...
            return Ok($"{playerName} has an Chance of Hitting {averageRunsScored} Runs in the Next Match");
        }
        ...
        {
            (string playerName, int averageRunsScored) = _playerRepository.GetAverageRunsScored(name);
            //var averageRunsScored = _playerRepository.GetAverageRunsScored(name);

            if (playerName =="" && averageRunsScored == 0)
            {
                return NotFound();
            }
        ...
        {
            (string playerName, int averageWicketsTaken) = _playerRepository.GetAverageWicketsTaken(id);
            ...
            return Ok($"{playerName} has an Chance of Picking up {averageWicketsTaken} Wickets in the Next Match");
        }
        ...
        {
            (string playerName, int averageWicketsTaken) = _playerRepository.GetAverageWicketsTaken(name);
            //var averageWicketstaken = _playerRepository.GetAverageWicketsTaken(name);

            if (playerName =="" && averageWicketsTaken == 0)
            {
                return NotFound();
            }
        ...
        /// <summary>
        /// Returns the list of top 10 players by strike-rate minimum 300 Runs Scored
        /// </summary>
        ...
        public ActionResult<IEnumerable<Player>> SearchByName(string name)
        {
            try
            {
        ...
        // /// <summary>
        // /// Calculates the Player Boundary hitting for number of balls player faced.
        // /// </summary>
        // /// <returns>Boundarypercentage</returns>
        // // GET api/player/boundarypercentage
        // [HttpGet("boundarypercentage")]
        // public ActionResult<IEnumerable<PlayerBoundaryPercentage>> GetBoundaryPercentage()
        // {
        ...
        //     {
        //         //condition to calculate if player has only scored runs
        ...
        //     return Ok(playerBoundaryPercentages);
        // }

        /// <summary>
        /// Search by Player name and Calculates the Player Boundary hitting for number of balls player faced.
        /// </summary>
        /// <param name="search"></param>
        /// <returns>Boundary Percentage</returns>
        // GET api/player/boundarypercentage?search={playerName}
        [HttpGet("BoundaryPercentageByName")]
        public ActionResult<IEnumerable<PlayerBoundaryPercentage>> GetBoundaryPercentagebyName(string search)
        {
            ...
            // Get Players from the Database
            List<Player> players = _playerRepository.GetPlayers().ToList();
        ...
        // DTO for returning boundary percentage
        public class PlayerBoundaryPercentage
        {
